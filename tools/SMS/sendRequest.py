...
class Service:
    ...
    def sendMessage(self, phone):
        ...
        # Create suffixes
        okay = f"{Fore.YELLOW}Numaranın ({getDomain(url)}) >> Anası sikildi{Fore.RESET}"
        error = f"{Fore.MAGENTA}Numaranın ({getDomain(url)}) >> TÜH başarısız :D{Fore.RESET}"
        ...
        try:
            ...
            print(f"{Fore.RED}[Bağlantın kesildi amk] {error}")
            ...
            if r.status_code == 200:
                print(f"{Fore.GREEN}[Başarılı amk] {okay}")
            elif r.status_code == 429:
                print(f"{Fore.RED}[Birazcık İstek] {error}")
            else:
